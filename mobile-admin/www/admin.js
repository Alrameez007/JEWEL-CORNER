...
refreshAppButton?.addEventListener(
  "click",
  () => {
...
loginForm.addEventListener(
 ...
 try {
  ...
  /*
     Start a fresh activity session BEFORE Firebase
  ...
  */
  ...
  /*
     Login failed, so this was not a valid
     authenticated activity session.
  */
...
function getLastActivityTime() {
  ...
  return Number.isFinite(savedTime)
    ? savedTime
    : 0;
...
/* ---------------------------------------------------------
   CHECK WHETHER 2 MINUTES HAVE ALREADY PASSED
   --------------------------------------------------------- */
...
/* ---------------------------------------------------------
   USER ACTIVITY WHILE APP IS OPEN
   --------------------------------------------------------- */
...
/* ---------------------------------------------------------
   APP BACKGROUND / RESUME PROTECTION
   --------------------------------------------------------- */
  ...
  async () => {
    ...
    if (document.hidden) {
      ...
      /*
         ...
         Stop relying on the JavaScript timer because
         Android may suspend it.
      */
    ...
    /*
       ...
       Check how long it has been since the
       admin last used the app.
    */
...
onAuthStateChanged(
  ...
  async user => {
    ...
    if (user) {
      ...
      /*
         IMPORTANT:
         Check the EXISTING activity timestamp before
         showing the dashboard.

         This catches an app that was force-closed and
         reopened after the 2-minute limit.
      */
      ...
      /*
         Do NOT call resetInactivityTimer() here because
         that would overwrite the existing timestamp.

         Continue only with the remaining allowed time.
      */
